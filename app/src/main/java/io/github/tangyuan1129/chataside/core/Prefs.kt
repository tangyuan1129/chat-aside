...
class Prefs(context: Context, prefsName: String = PREFS_MAIN) {
    ...
    /**
     * Which presentation the overlay uses. Two products on one core:
     *
     * - [MODE_CHOOSER] — a game-style strip of candidate replies. The judgment
     *   is collapsed to a single danger dot; the panel exists to be picked from.
     * - [MODE_ADVISOR] — a read-only judgment sheet (danger, true intent, what
     *   they need, best action). No candidates by default; you write your own.
     *
     * Default is chooser: on a fresh install the fastest path to a useful
     * action is picking a reply, not reading a report.
     */
    var overlayMode: String
        get() = sp.getString(K_OVERLAY_MODE, MODE_CHOOSER) ?: MODE_CHOOSER
        set(v) = sp.edit().putString(K_OVERLAY_MODE, v.trim()).apply()

    /** Convenience read; call sites should not string-compare directly. */
    val isChooser: Boolean get() = overlayMode != MODE_ADVISOR
    val isAdvisor: Boolean get() = overlayMode == MODE_ADVISOR

    /**
     * Chooser mode only. A first tap fills the input box (the v1.3 behaviour);
     * a second tap on the *same* option inside [chooserDoubleTapWindowMs]
     * presses send. Off means "fill only, forever" — the upstream guarantee.
     *
     * Sending is the one irreversible action this app takes on the user's
     * behalf, so it is gated behind both this switch and the window.
     */
    var chooserDoubleTapSend: Boolean
        get() = sp.getBoolean(K_CHOOSER_DBL_SEND, true)
        set(v) = sp.edit().putBoolean(K_CHOOSER_DBL_SEND, v).apply()

    /** How long the first tap stays armed for the second one (milliseconds). */
    var chooserDoubleTapWindowMs: Int
        get() = sp.getInt(K_CHOOSER_DBL_WINDOW, 2500).coerceIn(1000, 6000)
        set(v) = sp.edit().putInt(K_CHOOSER_DBL_WINDOW, v.coerceIn(1000, 6000)).apply()

    /**
     * Advisor mode: also draft candidates, shown folded as reference text with
     * no fill button. Default false — skipping the generative call plus the
     * Jev ranking call is roughly two thirds of the per-analysis cost, and it
     * is the whole reason this mode exists.
     */
    var advisorGenerateReplies: Boolean
    ...
    companion object {
        ...
        private const val K_OVERLAY_MODE = "overlay_mode"
        ...
        const val MODE_ADVISOR = "advisor"
