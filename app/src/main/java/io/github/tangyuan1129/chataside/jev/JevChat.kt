...
object JevChat {
    ...
    fun systemPrompt(): String = buildString {
        ...
        appendLine("  Also include \"probabilities\": your 0.0-1.0 estimate for EVERY allowed key of that")
        appendLine("  question, summing to about 1. Spread the mass across the plausible options instead of")
        appendLine("  putting it all on one — the differences between options are the point.")
        ...
        appendLine("- Judge THIS transcript, not what similar conversations usually look like.")
        appendLine("- Two different conversations must not get identical answers: if the threads differ in")
        appendLine("  tone, stakes, or what the other person wants, the answers and spreads must differ too.")
        appendLine("- Weigh the exact wording of the last 2-3 messages most heavily; earlier context modulates.")
        appendLine()
